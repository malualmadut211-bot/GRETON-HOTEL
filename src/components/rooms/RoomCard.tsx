...
export function RoomCard({ room, isFavorite, toggleFavorite, onViewDetails, onCompare, isCompared, viewMode, isSelected }: RoomCardProps) {
  ...
  return (
    <div 
      ...
      className={cn(
        "group relative bg-transparent border border-white/20 flex flex-col transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:-translate-y-3 hover:shadow-[0_20px_50px_rgba(0,0,0,0.15)] will-change-transform",
    ...
    >
      ...
      <div className={cn(
          ...
          {room.images.map((_, idx) => (
            <div 
              ...
              className={cn(
                ...
                idx === currentImg ? "w-6 bg-transparent border border-white/20" : "w-2 bg-white/50 hover:bg-transparent/80"
              )}
            />
          ))}
        </div>
        ...
        <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
           <button onClick={prevImg} className="w-10 h-10 rounded-full bg-transparent flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all text-white shadow-sm">
             <ChevronLeft size={20} />
           </button>
           <button onClick={nextImg} className="w-10 h-10 rounded-full bg-transparent flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all text-white shadow-sm">
        ...
        <div className="absolute top-4 right-4 flex items-center gap-3 z-10">
           ...
           <button 
             ...
             className={cn(
               ...
               isFavorite ? "bg-transparent border border-white/20" : "bg-white/90 animate-[pulseUnfavorited_1.5s_infinite]"
             )}
           >
             <Heart size={20} className={cn("transition-all duration-400", isFavorite ? "fill-[#e63946] text-[#e63946]" : "text-white")} />
             {showHeartBurst && (
      ...
      <div className={cn("p-[24px_28px_28px] flex-1 flex flex-col gap-4", viewMode === 'list' && "justify-between")}>
        ...
        <div>
          <motion.h3 
            ...
            className="text-[22px] md:text-[26px] font-semibold leading-[1.2] text-white tracking-[-0.5px] mb-2 hover:text-gold transition-colors cursor-pointer"
          >
            {room.name}
          </motion.h3>
          ...
          <motion.p 
            ...
            className={cn(
              "text-[15px] leading-[1.6] text-white/70 overflow-hidden text-ellipsis transition-all duration-300",
           ...
           {room.amenities.slice(0, viewMode === 'list' ? 8 : 6).map((item, idx) => {
             ...
             return Icon ? (
               <div key={idx} className="flex flex-col items-center gap-1.5 relative group/icon transition-transform duration-200 ease-out hover:-translate-y-1">
                 <Icon size={24} strokeWidth={1.5} className="text-white/70 group-hover/icon:text-gold group-hover/icon:scale-110 transition-all" />
                 <span className="text-[11px] text-white/60 uppercase tracking-[0.5px] group-hover/icon:text-gold group-hover/icon:font-medium">{item}</span>
               </div>
             ) : null;
           })}
        </div>
        ...
        <div className="flex flex-wrap gap-6 my-2">
           <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.6, duration: 0.4 }} className="flex items-center gap-2">
             ...
             <span className="text-[15px] font-semibold text-white">{room.size}</span>
             <span className="text-[13px] text-white/60">m²</span>
           </motion.div>
           <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.68, duration: 0.4 }} className="flex items-center gap-2">
             ...
             <span className="text-[15px] font-semibold text-white">Max {room.occupancy}</span>
             <span className="text-[13px] text-white/60">Guests</span>
           </motion.div>
        </div>

        {/* Pricing & Actions */}
        <div className={cn(
          "mt-auto pt-5  flex gap-4",
          viewMode === 'grid' ? "flex-col xl:flex-row xl:items-end justify-between" : "flex-row items-center justify-between"
        )}>
          {/* Price */}
          <div className="flex flex-col gap-1">
            <span className="text-[12px] text-white/60 uppercase tracking-[0.5px]">From</span>
            <div className="flex items-baseline">
              <span className="text-[28px] md:text-[32px] font-bold text-white tracking-[-1px] leading-none">${room.price}</span>
              <span className="text-[14px] text-white/60 ml-1">/night</span>
            </div>
          </div>
          ...
          <div className="flex gap-3 w-full xl:w-auto">
            <button 
              onClick={(e) => { e.stopPropagation(); onViewDetails(); }}
              className="flex-1 xl:flex-initial p-[14px_28px] bg-white/10 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.2)] text-white  border-2 border-[#1a1a1a] rounded-lg font-semibold text-[14px] uppercase tracking-[0.8px] text-white relative overflow-hidden group/view hover:-translate-y-[2px] hover:shadow-[0_6px_20px_rgba(0,0,0,0.1)] transition-all duration-300"
            >
