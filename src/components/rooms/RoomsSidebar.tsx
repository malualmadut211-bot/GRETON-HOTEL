...
export function RoomsSidebar({ selectedRoomId, roomsData, initialCheckIn, initialCheckOut, initialGuests }: RoomsSidebarProps) {
  ...
  return (
    <>
      <div className={cn(
        "hidden lg:block sticky top-[140px] bg-white rounded-[20px] shadow-[0_12px_48px_rgba(0,0,0,0.15)] border border-black/5 overflow-hidden transition-all duration-600 ease-[cubic-bezier(0.4,0,0.2,1)] z-40 transform-gpu",
        !selectedRoomId && "opacity-50 blur-[2px] translate-x-[20px]"
      )}>
        {/* Active Selection */}
        <div className="p-6 border-b border-[#f0f0f0]">
          <h3 className="text-[20px] font-semibold text-[#1a1a1a] mb-2">{room ? "Your Selection" : "No Room Selected"}</h3>
          {room ? (
            <>
              <div className="w-full h-[180px] rounded-xl overflow-hidden mb-4 relative cursor-pointer group">
                 ...
                 <span className="absolute top-2 left-2 bg-charcoal/80 backdrop-blur text-white text-[10px] uppercase font-bold px-2 py-1 rounded">
                   {room.category}
                 </span>
              </div>
              <h4 className="text-[20px] font-semibold text-[#1a1a1a] mb-2">{room.name}</h4>
              <p className="text-[13px] text-gold underline cursor-pointer hover:opacity-80 transition-opacity">Change Room</p>
            </>
          ) : (
            <p className="text-gray-400 text-sm">Please select a room to view pricing details and proceed.</p>
          )}
        </div>
        ...
        {room && (
          <>
            <div className="p-6 border-b border-[#f0f0f0]">
               <h4 className="text-[14px] font-semibold text-[#666] uppercase tracking-[0.5px] mb-4">Dates</h4>
               <div className="flex gap-3">
                 <div className="flex-1 p-[12px_16px] bg-[#f9f9f9] border-2 border-[#e0e0e0] rounded-[10px] text-[15px] text-[#1a1a1a] hover:border-gold hover:bg-white transition-colors cursor-pointer">
                    {displayCheckIn}
                 </div>
                 <div className="flex-1 p-[12px_16px] bg-[#f9f9f9] border-2 border-[#e0e0e0] rounded-[10px] text-[15px] text-[#1a1a1a] hover:border-gold hover:bg-white transition-colors cursor-pointer">
                    {displayCheckOut}
               ...
               <p className="mt-3 text-[13px] text-[#666]">{nights} nights selected {initialGuests ? `(${initialGuests})` : ''}</p>
            </div>

            <div className="p-6 bg-[#f9f9f9] border-b border-[#e0e0e0] space-y-3">
               <div className="flex justify-between text-[14px] text-[#666]">
                 <span>Room rate x {nights} nights</span>
                 <span className="font-medium text-[#1a1a1a]">${room.price * nights}</span>
               </div>
               <div className="flex justify-between text-[14px] text-[#666]">
                 <span>Taxes & fees</span>
                 <span className="font-medium text-[#1a1a1a]">${taxes}</span>
               </div>
               <div className="pt-4 mt-4 border-t-2 border-[#e0e0e0] flex items-end justify-between">
                 <span className="text-[16px] font-semibold text-[#333]">Total</span>
            ...
            <div className="p-[20px_24px] bg-[#f9f9f9] text-center">
               <p className="text-[14px] text-[#666] mb-3">Need help with your booking?</p>
      ...
      <div className={cn(
        "lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.15)] border-t border-[#e0e0e0] p-[16px_20px_24px] transition-transform duration-400 ease-[cubic-bezier(0.4,0,0.2,1)]",
        room ? "translate-y-0" : "translate-y-full"
      )}>
        {room && (
          <div className="flex items-center justify-between gap-4">
             <div className="flex flex-col flex-1">
               <p className="text-[12px] text-[#999] uppercase tracking-[0.5px]">Total ({nights} nights)</p>
               <p className="text-[24px] font-bold text-gold tracking-[-0.5px]">${total}</p>
               <p className="text-[12px] text-[#666] underline mt-1 cursor-pointer">View Details</p>
             </div>
