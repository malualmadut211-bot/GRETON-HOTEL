...
export default function ReviewsSocialCTA() {
  ...
  return (
    <>
      {/* Testimonials */}
      <section className="bg-transparent - ] py-[80px] md:py-[120px] px-6 md:px-20 relative overflow-hidden">
         <div className="max-w-[1400px] mx-auto relative z-10">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, ease: "backOut" }} className="text-center mb-16">
               <h2 className="font-serif text-[3rem] font-bold text-white mb-2">What Our Guests Say</h2>
               ...
               <p className="font-sans text-[1.125rem] text-white/80">333 Reviews</p>
            </motion.div>

            <div className="max-w-[800px] mx-auto bg-white/10 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.2)] text-white  rounded-3xl p-[40px] md:p-[60px] md:px-[80px] shadow-[0_16px_48px_rgba(0,0,0,0.08)] relative min-h-[400px] flex flex-col justify-center">
              ...
              <AnimatePresence mode="popLayout">
                 <motion.div key={currentReview} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1, transition:{ duration: 0.8, ease: "easeInOut" } }} exit={{ opacity: 0, scale: 0.95, transition: { duration: 0.8, ease: "easeInOut" } }} className="text-center relative z-10">
                   <p className="font-sans text-[1.125rem] md:text-[1.25rem] text-white/95 leading-[1.8] mb-10 max-w-[700px] mx-auto">
                   ...
                   <div className="flex items-center justify-center gap-5 flex-col md:flex-row">
                     ...
                     <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1, transition:{ delay: 0.4, duration: 0.5 } }} className="text-center md:text-left">
                       <h4 className="font-sans text-[1.125rem] font-semibold text-white mb-1">{REVIEWS[currentReview].name}</h4>
                       <p className="font-sans text-[0.9375rem] text-white/80">{REVIEWS[currentReview].loc}</p>
                     </motion.div>
                   </div>
            ...
            <div className="flex justify-center gap-4 mt-6">
              {REVIEWS.map((_, i) => (
                 <button key={i} onClick={() => setCurrentReview(i)} className={cn("w-3 h-3 rounded-full transition-all duration-300", currentReview === i ? "bg-gold scale-125 shadow-[0_0_8px_rgba(212,175,55,0.5)]" : "bg-black/20 hover:scale-110 hover:bg-white/20")} />
              ))}
            </div>
         </div>
      </section>
      ...
      {/* Final CTA */}
      <section className="bg-transparent relative min-h-[600px] flex items-center justify-center overflow-hidden py-20 px-6">
            ...
            <div className="flex flex-wrap justify-center gap-6 md:gap-10">
               {["Free Cancellation", "Best Price Guarantee", "Secure Booking"].map((badge, i) => (
                  <motion.div key={i} initial={{ y: 30, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.7 + i * 0.15 }} className="flex flex-col items-center">
                     <div className="w-10 h-10 rounded-full bg-transparent border border-white/30 flex items-center justify-center text-gold mb-3">
                       <CheckCircle size={20} />
                     </div>
