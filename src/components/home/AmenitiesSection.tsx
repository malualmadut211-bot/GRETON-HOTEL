  ...
  return (
    <section className="bg-transparent py-[80px] md:py-[120px] px-6 md:px-20 overflow-hidden">
      <div className="max-w-[1400px] mx-auto">
        ...
        >
          <h2 className="font-serif text-[2.5rem] font-bold text-white mb-4">World-Class Amenities</h2>
          <p className="font-sans text-[1.125rem] text-white/80">Everything you need for an exceptional stay</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {AMENITIES.map((am, index) => (
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-xl rounded-xl p-10 text-center border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.2)] hover:-translate-y-2.5 hover:border-gold hover:shadow-[0_12px_40px_rgba(212,175,55,0.4)] hover:bg-white/20 transition-all duration-400 group cursor-default"
            >
              ...
              <h3 className="font-sans text-xl font-semibold text-white mb-3 group-hover:text-gold transition-colors duration-300">{am.title}</h3>
              <p className="font-sans text-[0.9375rem] text-white/80 leading-relaxed">{am.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
