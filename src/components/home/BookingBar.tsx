...
export default function BookingBar() {
  ...
  return (
    <div className={cn("flex justify-center w-full z-40 px-6 md:px-0 transition-all duration-500", isSticky ? "fixed top-[70px] lg:top-[90px] left-0 right-0 px-0" : "relative -top-[50px]")}>
      <div 
        className={cn(
          "transition-all duration-400 ease-[cubic-bezier(0.4,0,0.2,1)] bg-white/95 backdrop-blur-[20px] saturate-[1.8] border border-white/30 shadow-[0_8px_32px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col md:flex-row items-center gap-4",
          isSticky 
      ...
      >
        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex flex-col group focus-within:ring-2 ring-gold/50 rounded-lg p-2 transition-all">
            <span className={cn("font-semibold text-gray-500 uppercase tracking-widest", isSticky ? "text-[10px]" : "text-xs font-sans")}>Check In</span>
            <div className="flex items-center gap-2 mt-1">
              <Calendar className="text-gold" size={isSticky ? 16 : 20} />
              <input type="date" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} className={cn("bg-transparent outline-none text-charcoal font-medium w-full font-sans", isSticky ? "text-sm" : "text-lg")} />
            </div>
          </div>
          <div className="flex flex-col group focus-within:ring-2 ring-gold/50 rounded-lg p-2 transition-all">
            <span className={cn("font-semibold text-gray-500 uppercase tracking-widest font-sans", isSticky ? "text-[10px]" : "text-xs")}>Check Out</span>
            <div className="flex items-center gap-2 mt-1">
              <Calendar className="text-gold" size={isSticky ? 16 : 20} />
              <input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} className={cn("bg-transparent outline-none text-charcoal font-medium w-full font-sans", isSticky ? "text-sm" : "text-lg")} />
            </div>
          </div>
          <div className="flex flex-col group focus-within:ring-2 ring-gold/50 rounded-lg p-2 transition-all relative">
            <span className={cn("font-semibold text-gray-500 uppercase tracking-widest font-sans", isSticky ? "text-[10px]" : "text-xs")}>Guests</span>
            <div className="flex items-center justify-between mt-1 cursor-pointer">
              <div className="flex items-center gap-2">
                <Users className="text-gold" size={isSticky ? 16 : 20} />
                <select value={guests} onChange={(e) => setGuests(e.target.value)} className={cn("bg-transparent outline-none text-charcoal font-medium font-sans appearance-none", isSticky ? "text-sm" : "text-lg")}>
              ...
              <ChevronDown className="text-charcoal/50" size={16} pointerEvents="none" />
            </div>
          </div>
        </div>
