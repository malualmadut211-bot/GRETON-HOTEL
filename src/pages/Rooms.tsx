...
import { SpecialOfferBanner } from "@/components/rooms/SpecialOfferBanner";
...
export default function Rooms() {
  ...
  return (
    <div className="bg-[#fcf8f2] min-h-screen pb-24 font-sans relative">
      
      {/* 1. HERO SECTION */}
      <section className="relative h-screen min-h-[600px] flex flex-col justify-center overflow-hidden bg-[#1A1A1A]">
        <motion.div style={{ y: yBg, scale: scaleBg }} className="absolute inset-0 z-0 origin-center">
          <img 
            src={ROOMS_DATA[1].images[0]} 
            alt="Luxury bedroom" 
        ...
        <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 mt-20">
          ...
          <motion.div 
             ...
             className="inline-flex bg-white/10 backdrop-blur-[20px] rounded-full p-2 border border-white/20 relative"
          >
             {CATEGORIES.map(cat => {
               ...
               return (
                 <button
                   ...
                   className={cn(
                     ...
                     isActive ? "text-[#1A1A1A]" : "text-white/70 hover:text-white hover:bg-white/10"
                   )}
                 >
                   {isActive && (
                     <motion.div 
      ...
      <AnimatePresence>
        {isSticky && (
          <motion.div 
            ...
            className="fixed top-[70px] lg:top-[90px] left-0 right-0 z-40 bg-white/70 backdrop-blur-[20px] saturate-[180%] border-b border-white/30 shadow-[0_8px_32px_rgba(0,0,0,0.08)] py-3 px-6"
          >
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isActive ? "bg-gradient-to-br from-[#1A1A1A] to-[#2D2D2D] text-white shadow-[0_6px_20px_rgba(0,0,0,0.15)]" : "bg-transparent text-gray-500 border border-black/10 hover:bg-black/5 hover:-translate-y-[2px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]"
                        )}
                      >
                        {cat}
                        {isActive && (
               ...
               <div className="hidden md:flex items-center gap-6">
                 ...
                 <div className="relative">
                   <button 
                     ...
                     className={cn(
                       "flex items-center gap-3 px-4 py-2.5 bg-white border rounded-lg text-[14px] font-medium transition-all duration-200",
                   ...
                   >
                     ...
                     <span className="text-[#1A1A1A]">{sortBy}</span>
                     <ChevronDown size={16} className={cn("text-gray-400 transition-transform duration-300", isSortOpen && "rotate-180")} />
                   </button>
                   ...
                   <AnimatePresence>
                     {isSortOpen && (
                       <motion.div 
                         ...
                         className="absolute top-[calc(100%+8px)] right-0 w-[240px] bg-white border border-black/5 rounded-xl shadow-[0_12px_40px_rgba(0,0,0,0.15)] p-2 z-50 transform-origin-top"
                       >
                          {SORT_OPTIONS.map((opt, i) => (
                            <motion.button
                              ...
                              className={cn(
                                ...
                                sortBy === opt ? "bg-gold/10 text-gold" : "text-gray-600 hover:bg-gold/10 hover:text-gold hover:translate-x-1"
                              )}
                            >
                              {opt}
                 ...
                 {/* View Toggle */}
                 <div className="flex bg-white border border-gray-200 rounded-lg p-1 relative">
                    <motion.div 
                      className="absolute top-1 bottom-1 w-[40px] bg-gray-100 rounded-md z-0 shadow-[0_2px_8px_rgba(0,0,0,0.08)]"
                      animate={{ translateX: viewMode === 'grid' ? 0 : 44, backgroundColor: viewMode === 'list' ? 'rgba(212,175,55,0.1)' : '#f5f5f5' }}
                      transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
                    />
                    <button 
                      onClick={() => setViewMode('grid')}
                      className="w-[40px] h-[40px] flex md:hidden lg:flex items-center justify-center rounded-md z-10 text-gray-500 relative transition-colors duration-300"
                    >
                      <LayoutGrid size={20} className={cn("transition-all duration-400", viewMode === 'grid' && "text-[#1A1A1A] stroke-2")} />
                    </button>
                    <button 
                      onClick={() => setViewMode('list')}
                      className="w-[40px] h-[40px] flex md:hidden lg:flex items-center justify-center rounded-md z-10 text-gray-500 relative transition-colors duration-300"
                    >
                      <List size={20} className={cn("transition-all duration-400", viewMode === 'list' && "text-[#1A1A1A] stroke-2")} />
                    </button>
                 </div>
               </div>
            </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 mt-[80px] flex flex-col lg:flex-row gap-12 relative items-start">
          ...
          >
            <AnimatePresence mode="popLayout">
              {filteredRooms.flatMap((room, i) => {
                ...
                {/* Inject Special Offer Banner dynamically (e.g. after 2nd item) */}
                if (i === 2) {
                  items.push(
                    <motion.div 
                      key="special-offer-banner"
                      layout
                      initial={{ opacity: 0, x: 100 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      viewport={{ once: true, margin: "-10%" }}
                      transition={{ duration: 0.8, ease: "easeOut" }}
                      className={cn(viewMode === "grid" && "md:col-span-2")}
                    >
                      <SpecialOfferBanner />
                    </motion.div>
                  );
        ...
        {/* 10. SIDEBAR / STICKY FOOTER */}
        <div className="lg:w-[360px] shrink-0 sticky top-[140px] z-30">
           <RoomsSidebar 
             selectedRoomId={selectedForBooking} 
      ...
      <AnimatePresence>
        {selectedRoomsToCompare.length > 0 && !isComparing && (
          <motion.div 
            ...
            className="fixed left-0 right-0 z-[999] bg-white border-t-[3px] border-gold shadow-[0_-8px_32px_rgba(0,0,0,0.15)] p-[20px_40px] flex items-center justify-between gap-4 overflow-x-auto"
          >
                ...
                {selectedRoomsToCompare.map((id, i) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className="w-[200px] h-[80px] bg-[#f9f9f9] rounded-xl flex items-center gap-3 p-3 relative shrink-0"
                    >
                      <img src={room.images[0]} alt={room.name} className="w-[56px] h-[56px] rounded-lg object-cover" />
                      <div className="flex flex-col">
                        <span className="text-[14px] font-semibold text-[#1A1A1A] truncate max-w-[100px]">{room.name}</span>
            ...
            <div className="flex items-center gap-4 shrink-0 bg-white shadow-[-10px_0_10px_white] pl-4">
               <button 
                 onClick={() => setSelectedRoomsToCompare([])}
                 className="text-[14px] text-[#999] underline hover:text-[#e63946] hover:decoration-[#e63946] transition-colors p-2"
               >
                 Clear All
               </button>
               <button 
